---
name: pixem-optimize-stitch-route
...
1. Call `pixem_get_project_info` to get the list of layers in the active project.
2. For each layer:
   * Call `pixem_get_layer_details(layer_uuid)` to retrieve the list of partitions, their UUIDs, and colors.
   * Call `pixem_get_layer_image(layer_uuid)` to fetch the base64-encoded PNG image of the layer.
...
1. Call **`pixem_set_partition_route(layer_uuid, partition_uuid, route_json)`** to update the partition's route in Pixem.
